...
// Import file GeoJSON (sungai Martapura dekat Barito)
import sungaiMartapuraGeoJSON from './geojson/sungaimartapura-dekatbarito.json';
...
const MapGrid = ({ riverBounds, stepMeters = 275, enabled = true, marginDeg = 0.05 }) => {
  ...
  useEffect(() => {
    ...
    // Hitung batas minimum dan maksimum dari sungai
    ...
    // Perluas area dengan margin agar grid mencakup area yang lebih luas (full)
    minLat -= marginDeg;
    maxLat += marginDeg;
    minLng -= marginDeg;
    maxLng += marginDeg;
...
// Ekstrak koordinat sungai Martapura
const batasSungai = extractCoordinates(sungaiMartapuraGeoJSON);

// Fungsi untuk menghitung titik tengah
const calculateCenter = (coordinates) => {
  if (coordinates.length === 0) return [-3.3478, 114.5547];
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 relative">
          ...
          >
            ...
            {/* Grid kotak-kotak FULL - mencakup area lebih luas */}
            <MapGrid 
              riverBounds={batasSungai}
              stepMeters={gridSizeMeters}
              enabled={gridEnabled}
              marginDeg={0.05} // margin sekitar 5.5 km di setiap sisi
            />
            
            {/* Batas sungai dari GeoJSON */}
            <GeoJSON 
              data={sungaiMartapuraGeoJSON}
              style={polygonStyle}
              ref={geojsonRef}
            />
